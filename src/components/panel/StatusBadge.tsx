...
export default function StatusBadge({
  ...
  const map: Record<Variant, string> = {
    ...
    open: "bg-brand-primary-light text-brand-primary border-brand-primary/20",
    answered: "bg-success/10 text-success border-success/20",
    closed: "bg-neutral-100 text-neutral-700 border-neutral-200",
  };
  return (
    <span
      className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold ${map[value]}`}
    >
      {value}
    </span>
  );
